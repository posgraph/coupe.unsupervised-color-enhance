...
def generator_resnet_affine(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        d2 = tf.nn.relu(instance_norm(d2, 'g_d2_bn'))
        d2 = tf.pad(d2, [[0, 0], [3, 3], [3, 3], [0, 0]], "REFLECT")
        pred = (conv2d_last(d2, 12, 7, 1, padding='VALID', name='g_pred_c'))
        row1, row2, row3 = tf.split(pred, 3, 3)
        image_homo = tf.pad(image, [[0, 0], [0, 0], [0, 0], [0, 1]], "CONSTANT", constant_values=1.0)
        affined1 = tf.reduce_sum(tf.multiply(image_homo, row1), axis=3, keep_dims=True)
        affined2 = tf.reduce_sum(tf.multiply(image_homo, row2), axis=3, keep_dims=True)
        affined3 = tf.reduce_sum(tf.multiply(image_homo, row3), axis=3, keep_dims=True)
        out = tf.concat((affined1, affined2, affined3), axis=3)
        return out
